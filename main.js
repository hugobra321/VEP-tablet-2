...
function buildFinalText(data) {
  ...
  var finalText = 'A partir das constatações, o MM Juiz da Vara de Execução de Penas no Regime Fechado, com espeque no art. 66, VII, da LEP, proferiu as seguintes determinações:\n\n';
...
function finalizeAndSavePDF(doc, marginLeft) {
  var imageInput = document.getElementById('fotoInput');
  ...
  for (var i = 0; i < files.length; i++) {
    (function(file) {
      sequence = sequence.then(function() { return readFileAsDataURL(file); }).then(function(dataUrl) {
  ...
  sequence.then(function(){ addFooterAndSave(doc); }).catch(function(){ addFooterAndSave(doc); });
}

function addFooterAndSave(doc) {
  var totalPages = doc.getNumberOfPages();
  for (var i = 1; i <= totalPages; i++) {
    doc.setPage(i); doc.setFontSize(10); doc.text('Página ' + i + ' de ' + totalPages, 105, 290, { align: 'center' }); doc.text('Assinatura do Juiz e data lançadas digitalmente', 195, 290, { align: 'right' });
  }
